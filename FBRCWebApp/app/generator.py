...
class FlowCellGenerator:
    ...
    def __init__(
        ...
        # FIX: Added endplate_z here so it can catch the parameter from FastAPI
        ...
        self.endplate_z = endplate_z
        self.use_hose_barbs = use_hose_barbs
        self.barb_total_height = barb_total_height
        self.barb_inner_radius = barb_inner_radius
        self.barb_stem_radius = barb_stem_radius
        self.barb_outer_radius = barb_outer_radius
        self.barb_count = barb_count
        self.barb_base_fillet = barb_base_fillet
    ...
    def _get_tangent_points(circle_center, radius, target_point):
        ...
        if dist < radius: raise ValueError("Target point is inside the circle")
    ...
    def _layout_rectangles(Ltotal, rect_width, growth="exp", rate=0.2, lead_margin=None, trail_margin=None, min_first_gap=2, max_gap=None):
        ...
        if Luse <= 0 or rect_width <= 0: return [], 0, 0.0, []
        ...
        for N in range(N_theoretical_max, 1, -1):
            ...
            if min(s0, s_last) >= min_first_gap:
                if max_gap is not None and max(s0, s_last) > max_gap: continue 
                bestN, best_s0 = N, s0
                break
    ...
    def _build_bolt_locations(self) -> List[Tuple[float, float]]:
        ...
        return ([(x, 0) for x in boltX] + [(x, self.plate_y) for x in boltX] + 
                [(0, y) for y in boltY] + [(self.plate_x, y) for y in boltY])

    def _build_channels_sketch(self) -> Sketch:
        aa_x_min, aa_x_max = (self.plate_x - self.electrode_x) / 2, (self.plate_x + self.electrode_x) / 2
        aa_y_min, aa_y_max = (self.plate_y - self.electrode_y) / 2, (self.plate_y + self.electrode_y) / 2
        dist_inlet_y, dist_outlet_y = aa_y_min - self.dist_len, aa_y_max + self.dist_len

        _, _, _, gaps_ascending = self._layout_rectangles(
            self.electrode_x, rect_width=self.rib_thickness, growth=self.dist_pattern, rate=self.dist_rate
        )
        ...
        with BuildSketch() as sketch:
            ...
            for width in gaps_ascending:
                ...
                with Locations((cx, finger_cy)): Rectangle(width, finger_h)
                current_x += width
    ...
    def _create_barb_fitting(self) -> Part:
        """Generates a solid hose barb."""
        with BuildPart() as b:
            Cylinder(radius=self.barb_stem_radius, height=self.barb_total_height, align=(Align.CENTER, Align.CENTER, Align.MIN))
            barb_section_h = self.barb_total_height * 0.7
            single_barb_h = barb_section_h / self.barb_count
            start_z = self.barb_total_height - barb_section_h
            for i in range(self.barb_count):
                z_pos = start_z + (i * single_barb_h)
                with Locations((0, 0, z_pos)):
                    Cone(bottom_radius=self.barb_outer_radius, top_radius=self.barb_stem_radius, height=single_barb_h, align=(Align.CENTER, Align.CENTER, Align.MIN))
        return b.part
    ...
    def generate_flow_frame(self) -> Part:
        ...
        with BuildSketch() as through_holes:
            with Locations(self._build_bolt_locations()): Circle(self.bolt_dia / 2)
            with Locations(self.dummy_coords + [self.inlet_coords, self.outlet_coords]): Circle(self.port_dia / 2)
            with Locations((self.plate_x/2, self.plate_y/2)): Rectangle(self.electrode_x, self.electrode_y)
    ...
    def generate_flow_field(self) -> Part:
        ...
        with BuildSketch() as wetted_areas:
            with Locations([self.inlet_coords, self.outlet_coords]): Circle(self.port_dia / 2)
            with Locations((self.plate_x/2, self.plate_y/2)): Rectangle(self.electrode_x, self.electrode_y)
        with BuildPart() as part:
            extrude(channel_sketch, amount=self.manifold_channel_depth)
            extrude(wetted_areas.sketch.moved(Location((0, 0, -self.manifold_wall_thickness))), amount=self.plate_z_flow)
        return part.part

    def generate_bipolar_current_collector(self) -> Sketch:
        plate_fillet = min(self.plate_x, self.plate_y) * self.fillet_ratio
        with BuildSketch() as sketch:
            with Locations((self.plate_x/2, self.plate_y/2)): Rectangle(self.plate_x, self.plate_y)
    ...
    def generate_end_plate(self, thickness=12.0) -> Part:
        ...
        with BuildPart() as part: extrude(profile, amount=thickness)
        return part.part

    def generate_end_plate_with_barbs(self, thickness=12.0) -> Part:
        fillet_r = min(self.plate_x + self.endplate_x_offset, self.plate_y + self.endplate_y_offset) * self.fillet_ratio
        
        # We only want barbs/holes on the two "Top" ports
        top_port_locs = [
            (self.plate_x * self.port_inset, self.plate_y * (1 - self.port_inset)),
            (self.plate_x * (1 - self.port_inset), self.plate_y * (1 - self.port_inset))
        ]

        with BuildPart() as solid_plate:
            with BuildSketch() as base_rect:
                with Locations((self.plate_x/2, self.plate_y/2)):
                    Rectangle(self.plate_x + self.endplate_x_offset, self.plate_y + self.endplate_y_offset)
                fillet(base_rect.vertices(), radius=fillet_r)
            extrude(base_rect.sketch, amount=thickness)

            barb_geometry = self._create_barb_fitting()
            with Locations(Location((0,0, thickness))):
                with Locations(top_port_locs): add(barb_geometry)

            base_edges = [
                e for e in solid_plate.edges().filter_by(GeomType.CIRCLE) 
                if math.isclose(e.center().Z, thickness, abs_tol=1e-3)
            ]
            if base_edges: fillet(base_edges, radius=self.barb_base_fillet)

            cut_depth = thickness + self.barb_total_height + 5.0
            with BuildSketch() as holes_sketch:
                with Locations(self._build_bolt_locations()): Circle(self.bolt_dia / 2)
                # Only cut through-holes for the barbed ports; the others are solid caps
                with Locations(top_port_locs): Circle(self.barb_inner_radius)
                    
            extrude(holes_sketch.sketch, amount=cut_depth, both=True, mode=Mode.SUBTRACT)

        return solid_plate.part
    ...
    def generate_current_collector(self) -> Sketch:
        ...
        with BuildSketch() as sketch:
            with Locations((base_cx, base_cy)): Rectangle(self.plate_x + self.cc_x_offset, self.plate_y + self.cc_y_offset)
            with Locations((tab_cx, tab_cy)): Rectangle(self.cc_tab_length, self.cc_tab_width)
            fillet(sketch.vertices(), radius=fillet_r)
            with BuildSketch(mode=Mode.SUBTRACT):
                with Locations(self._build_bolt_locations()): Circle(self.bolt_dia / 2)
                with Locations(self.dummy_coords + [self.inlet_coords, self.outlet_coords]): Circle(self.port_dia / 2)
                with Locations((tab_cx, tab_cy)): Circle(self.cc_tab_hole_radius)
        return sketch.sketch
